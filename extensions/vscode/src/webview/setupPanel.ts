...
export class SetupPanel {
  public static readonly viewType = "cgcSetup";
  private static currentPanel: SetupPanel | undefined;
  private readonly _panel: vscode.WebviewPanel;
  private _disposables: vscode.Disposable[] = [];

  public static createOrShow(context: vscode.ExtensionContext, client: CgcMcpClient) {
    ...
    SetupPanel.currentPanel = new SetupPanel(panel, context, client);
  }

  private constructor(panel: vscode.WebviewPanel, context: vscode.ExtensionContext, private readonly client: CgcMcpClient) {
    this._panel = panel;
    ...
    this._panel.webview.onDidReceiveMessage(
      async (message) => {
        switch (message.command) {
          case "test":
            await this._handleTest(message.executable, message.dbMode);
            break;
          case "save":
            await this._handleSave(
              message.executable,
              message.dbMode,
              message.pythonPackagePath,
              message.maxToolResponseTokens,
              message.neo4jUri
            );
            break;
        }
  ...
  private async _handleTest(executable: string, dbMode: string) {
    this._panel.webview.postMessage({ command: "testResult", status: "testing", message: "Starting test..." });
    
    // Temporarily override or use a dry-run start
    try {
      // We'll use a one-off spawn to check version/help
      const { exec } = require("child_process");
      const cmd = `${executable} --help`;
      ...
      exec(cmd, async (error: any, stdout: string, stderr: string) => {
        ...
        this._panel.webview.postMessage({ 
          command: "testResult", 
          status: "success", 
          message: "Executable found and responding! Now checking MCP connectivity..." 
        });

        // Try actual MCP start if possible
        try {
            await this.client.ensureStarted(); // This uses current settings, so test might be limited
            this._panel.webview.postMessage({ 
                command: "testResult", 
                status: "success", 
                message: "Perfect! CGC MCP is online and connected." 
            });
        } catch (mcpErr: any) {
            this._panel.webview.postMessage({ 
                command: "testResult", 
                status: "warning", 
                message: `Executable ok, but MCP failed: ${mcpErr.message}` 
            });
        }
      });
    } catch (err: any) {
      this._panel.webview.postMessage({ command: "testResult", status: "error", message: err.message });
    }
  }
  ...
  private async _handleSave(
    ...
    await cfg.update("executable", executable, vscode.ConfigurationTarget.Global);
    await cfg.update("databaseMode", dbMode, vscode.ConfigurationTarget.Global);
    await cfg.update("pythonPackagePath", pythonPackagePath ?? "", vscode.ConfigurationTarget.Global);
    await cfg.update("maxToolResponseTokens", maxToolResponseTokens ?? 0, vscode.ConfigurationTarget.Global);
    await cfg.update("neo4jUri", neo4jUri ?? "", vscode.ConfigurationTarget.Global);
    vscode.window.showInformationMessage("CGC settings saved. Restarting extension...");
  ...
  private _update() {
    ...
    this._panel.webview.html = this._getHtmlForWebview(executable, dbMode, pythonPackagePath, maxToolResponseTokens, neo4jUri);
  }
    ...
    <body>
        <div class="container">
            ...
            <div class="field">
                ...
                <input type="text" id="executable" value="${executable}" placeholder="e.g. /usr/local/bin/cgc or .venv/bin/cgc">
                <div class="hint">Absolute path to the 'cgc' CLI tool.</div>
            </div>
            ...
            <div class="field">
                ...
                <div class="hint">Target database for storing the code graph.</div>
            </div>
            <div class="field">
                <label>Python Package Path / uvx Path</label>
                <input type="text" id="pythonPackagePath" value="${pythonPackagePath}" placeholder="e.g. /usr/bin/uvx or custom PYTHONPATH">
            </div>
            <div class="field">
                <label>Neo4j URI</label>
                <input type="text" id="neo4jUri" value="${neo4jUri}" placeholder="bolt://localhost:7687">
            </div>
            <div class="field">
                <label>Max Tool Response Tokens</label>
                <input type="number" id="maxToolResponseTokens" value="${maxToolResponseTokens}" min="0">
            </div>
        ...
        <script>
            ...
            function test() {
                ...
                const dbMode = document.getElementById('dbMode').value;
                vscode.postMessage({ command: 'test', executable, dbMode });
            }

            function save() {
                const executable = document.getElementById('executable').value;
                const dbMode = document.getElementById('dbMode').value;
                const pythonPackagePath = document.getElementById('pythonPackagePath').value;
                const maxToolResponseTokens = Number(document.getElementById('maxToolResponseTokens').value || 0);
            ...
            window.addEventListener('message', event => {
                ...
                switch (message.command) {
                    case 'testResult':
                        ...
                        statusDiv.innerText = message.message;
                        break;
                    case 'saved':
                        statusDiv.className = 'status-success';
                        statusDiv.innerText = 'Settings saved and applied successfully!';
                        break;
                }
            });
        </script>
